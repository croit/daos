...
// 	}
// 	err = aiCache.Cache(netCtx, &mgmtpb.GetAttachInfoResp{}, scanResults)
// 	if err != nil {
...
func TestAgent_attachInfoCache_Cache(t *testing.T) {
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			err := tc.cache.Cache(context.TODO(), tc.input)

			common.CmpErr(t, tc.expErr, err)

			common.AssertEqual(t, tc.expCached, tc.cache.IsCached(), "unexpected result for IsCached()")
		})
	}
}
...
// 			var err error
// 			if aiCache.IsCached() {
// 				resp, err = aiCache.GetResponse(tc.numaNode)
...
// 			numDevs := len(aiCache.localNUMAFabric[tc.numaNode])
...
// 			err := aiCache.Cache(netCtx, resp, scanResults)
...
// 				var err error
// 				response[i], err = aiCache.GetResponse(tc.numaNode)
// 				common.AssertEqual(t, err, nil, "getResponse error")
// 			}
...
// 	defer wg.Done()
// 	resp, err := aiCache.GetResponse(numaNode)

...
type attachInfoCache struct {
	sync.Mutex // Caller is expected to lock and unlock around cache operations
...
func (c *localFabricCache) Cache(ctx context.Context, scan []*netdetect.FabricScan) {
	...
	for _, fs := range scan {
		...
		deviceAlias, err := netdetect.GetDeviceAlias(ctx, newIF.Name)
		if err != nil {
...
func (c *localFabricCache) GetDevice(numaNode int, netDevClass uint32) (*fabricInterface, error) {
	if !c.IsCached() {
		return nil, errors.New("no fabric data cached")
	}
...
func (c *localFabricCache) selectRemote() (int, int) {
	...
	if len(c.localNUMAFabric) == 0 {
		c.log.Infof("No network device available")
